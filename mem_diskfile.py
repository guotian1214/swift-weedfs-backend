...
from swift.obj.seaweedfs_operation import Seaweedfs_operation
import StringIO
...
class InMemoryFileSystem(object):
    ...
    def __init__(self):
        self._filesystem = {}
        self.seaweedfs = Seaweedfs_operation()
    ...
    def get_object(self, name):
        ...
        val = self.seaweedfs.GET(name)
        if val is None:
            fp, metadata = None, None
        else:
            fp, metadata = self.parse_seaweedfs_file(val)
        # else:
        #     fp, metadata = val
        return fp, metadata

    def parse_seaweedfs_file(self,val):
        """
        parse file download from seaweedfs
        :param val: seaweedfs file,fomat: str(metadata) + '\n' + obj_content
        :return:
        """
        metadata_index = val.find('\n')
        val = val.split('\n',1)
        return  StringIO.StringIO(val[1]),eval(val[0])


    def put_object(self, name, fp, metadata):
        """
        Store object into seaweedfs
        ...
        fp.seek(0)
        self.seaweedfs.PUT(name,str(metadata) + '\n' + fp.read())
    ...
    def del_object(self, name):
        ...
        """
        # import pydevd
        # pydevd.settrace('127.0.0.1', port=54321, stdoutToServer=True, stderrToServer=True)
        self.seaweedfs.DELETE(name)
